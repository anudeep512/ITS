...
public class OMSLibraryAddObjectTypesController implements Initializable {
    ...
    private ObservableList<ObjectType> getRecords() throws SQLException {
        ...
        String query ="select objecttype from libraryobjectclasses" ;
        try{
    ...
    public void addObjectType(ActionEvent event) throws SQLException {
        ...
        if(tfObjectType.getText().trim().length()>0){
            try{
                errorLabel.setVisible(false);
                Connection con = getConnection() ;
                String query = "create table library"+tfObjectType.getText().trim().toLowerCase()+"(" +
                ...
                String query2 = "Insert into libraryobjectclasses(objecttype) values(?) " ;
    ...
    public void deleteObjectType(ActionEvent event) throws SQLException {
        ...
        if(tfObjectType.getText().trim().length()>0){
            try{
                ...
                errorLabel.setVisible(false);
                Connection con = getConnection() ;
                String query = "drop table library"+tfObjectType.getText().trim().toLowerCase() ;
                Statement st = con.createStatement() ;
                st.executeUpdate(query);
                String querySV = "update librarysupervisortable set objectmaintained='null' where objectmaintained=? " ;
                ...
                String query3 = "Delete from libraryobjectclasses where objecttype=?" ;
