...
public class OMSLabSVTableController implements Initializable {
        ...
        public void setIithId(String iithId) {
                OMSLabSVTableController.iithId = iithId;
        }
        ...
        void deleteRecord(ActionEvent event) throws SQLException {
                ...
                if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0&& tfObjectMaintained.getText().trim().toLowerCase().length()!=0) {
                        ...
                        String svId = null;
                        String query0 = "Select iith_id from labsupervisortable where name = ? and contactnumber=?and objectmaintained=?";
                        ...
                        String query="delete from labsupervisortable where name=? and contactnumber=? and objectmaintained=? " ;
                        ...
                        String query2= "update labitemlist set supervisor='null' where supervisor=? and type=? " ;
        ...
        void updateRecord(ActionEvent event) throws SQLException {
                ...
                if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0 && tfObjectMaintained.getText().trim().toLowerCase().length()!=0 ) {
                        ...
                        if(tfObjectMaintained.getText().trim().equals(sv.getObjectClassUnderSupervision().trim())){
                                errorTypeError.setVisible(false);
                                String query="update labsupervisortable set name=? , contactnumber=? where name=? and contactnumber=? and objectmaintained=?" ;
                                Connection con = getConnection();
                                PreparedStatement st=con.prepareStatement(query) ;
                                st.setString(1,tfName.getText());
                                st.setString(2,tfContactNumber.getText());
        ...
        void insertRecord(ActionEvent event) throws SQLException {
                ...
                if(tfName.getText().trim().length()>0&&tfContactNumber.getText().trim().length()>0 && tfObjectMaintained.getText().trim().toLowerCase().length()>0){
                        errorInsert.setVisible(false);
                        errorLabel.setVisible(false);
                        String query = "SELECT * from labobjectclasses where objecttype = ? " ;
                       ...
                       if(resulSet.next()){
                               Connection con = getConnection() ;
                               String addQuery = "INSERT INTO labsupervisortable(name,fieldofoperations,objectmaintained,appointedby,contactnumber,password) values(?,?,?,?,?,?)" ;
                               ...
                               st.setString(6,"1234");
                               st.executeUpdate();
                               String queryForId = "SELECT id from labsupervisortable ORDER BY id DESC LIMIT 1" ;
                               ...
                               String queryUpdate = "Update labsupervisortable SET iith_id=?,iith_mailid=? Where id =? ";
        ...
        private ObservableList<Supervisor> getSVList() throws SQLException {
                ...
                Connection con = getConnection() ;
                String query = "SELECT * FROM labsupervisortable" ;
                try{
        ...
        public void filter(ActionEvent event) {
                ...
                if(tfFilterAppointedBy.getText().trim().length()>0 || tfFilterName.getText().trim().length()>0||tfFilterContactNumber.getText().trim().length()>0||tfFilterIithId.getText().trim().length()>0 ||tfFilterObjectMaintained.getText().trim().length()>0){
                        ...
                        try{
                                ...
                                Connection con = getConnection() ;
                                ObservableList<Supervisor> supervisors = FXCollections.observableArrayList();
                                String query = "SELECT * FROM labsupervisortable where iith_id LIKE ? and name LIKE ? and contactnumber LIKE ? and appointedby LIKE ? and objectmaintained LIKE ? order by iith_id ";
                                PreparedStatement st = con.prepareStatement(query);
        ...
        public void OnBack(ActionEvent event) throws IOException{
                ...
                errorFilterLabel.setVisible(false);
                errorInsert.setVisible(false);
                root = FXMLLoader.load(requireNonNull(getClass().getResource("OMSLabMode.fxml")));
