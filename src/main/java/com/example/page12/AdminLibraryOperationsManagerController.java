...
public class AdminLibraryOperationsManagerController implements Initializable {
    @FXML
    ...
    void deleteRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0) {
            errorLabel.setVisible(false);
            String query="delete from libraryoperationsmanagertable where name=? and contactnumber=? " ;
    ...
    void insertRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length()>0&&tfContactNumber.getText().trim().length()>0){
            errorLabel.setVisible(false);
            Connection con = getConnection() ;
            String addQuery = "INSERT INTO libraryoperationsmanagertable(name,fieldofoperations,appointedby,contactnumber,password) values(?,?,?,?,?)" ;
            ...
            st.setString(5,"1234");
            st.executeUpdate();
            String queryForId = "SELECT id from libraryoperationsmanagertable ORDER BY id DESC LIMIT 1" ;
            ...
            String queryUpdate = "Update libraryoperationsmanagertable SET iith_id=?,iith_mailid=? Where id =? ";
    ...
    void updateRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0) {
            errorLabel.setVisible(false);
            String query="update libraryoperationsmanagertable set name=? , contactnumber=? where name=? and contactnumber=?" ;
            Connection con = getConnection();
            PreparedStatement st=con.prepareStatement(query) ;
            st.setString(1,tfName.getText());
            st.setString(2,tfContactNumber.getText());
    ...
    public void filter(ActionEvent event) {
        ...
        if(tfFilterAppointedBy.getText().trim().length()>0 || tfFilterName.getText().trim().length()>0||tfFilterContactNumber.getText().trim().length()>0||tfFilterIithId.getText().trim().length()>0){
            ...
            try{
                ...
                String query = "SELECT * FROM libraryoperationsmanagertable where iith_id LIKE ? and name LIKE ? and contactnumber LIKE ? and appointedby LIKE ? order by iith_id";
                PreparedStatement st = con.prepareStatement(query);
    ...
    private ObservableList<OperationsManager> getOMList() throws SQLException {
        ObservableList<OperationsManager> operationsManagers = FXCollections.observableArrayList();
        Connection con = getConnection() ;
        String query = "SELECT * FROM libraryoperationsmanagertable" ;
        try{
