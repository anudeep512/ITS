...
public class OMSLabAddObjectTypesController implements Initializable {
    ...
    private ObservableList<ObjectType> getRecords() throws SQLException {
        ...
        String query ="select objecttype from labobjectclasses" ;
        try{
    ...
    public void addObjectType(ActionEvent event) throws SQLException {
        ...
        if(tfObjectType.getText().trim().length()>0){
            try{
                errorLabel.setVisible(false);
                Connection con = getConnection() ;
                String query = "create table lab"+tfObjectType.getText().trim().toLowerCase()+"(" +
                ...
                String query2 = "Insert into labobjectclasses(objecttype) values(?) " ;
    ...
    public void deleteObjectType(ActionEvent event) throws SQLException {
        ...
        if(tfObjectType.getText().trim().length()>0){
            try{
                ...
                errorLabel.setVisible(false);
                Connection con = getConnection() ;
                String query = "drop table lab"+tfObjectType.getText().trim().toLowerCase() ;
                Statement st = con.createStatement() ;
                st.executeUpdate(query);
                String querySV = "update labsupervisortable set objectmaintained='null' where objectmaintained=? " ;
                ...
                String query3 = "Delete from labobjectclasses where objecttype=?" ;
