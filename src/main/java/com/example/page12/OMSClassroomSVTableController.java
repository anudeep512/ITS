...
public class OMSClassroomSVTableController implements Initializable {
    ...
    public void setIithId(String iithId) {
        OMSClassroomSVTableController.iithId = iithId;
    }
    ...
    void deleteRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0&& tfObjectMaintained.getText().trim().toLowerCase().length()!=0) {
            ...
            String svId = null;
            String query0 = "Select iith_id from classsupervisortable where name = ? and contactnumber=?and objectmaintained=?";
            ...
            String query="delete from classsupervisortable where name=? and contactnumber=? and objectmaintained=? " ;
            ...
            String query2= "update classroomitemlist set supervisor='null' where supervisor=? and type=? " ;
    ...
    void insertRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length()>0&&tfContactNumber.getText().trim().length()>0 && tfObjectMaintained.getText().trim().toLowerCase().length()>0){
            errorInsert.setVisible(false);
            errorLabel.setVisible(false);
            String query = "SELECT * from classroomobjectclasses where objecttype = ? " ;
            ...
            if(resulSet.next()){
                Connection con = getConnection() ;
                String addQuery = "INSERT INTO classsupervisortable(name,fieldofoperations,objectmaintained,appointedby,contactnumber,password) values(?,?,?,?,?,?)" ;
                ...
                st.setString(6,"1234");
                st.executeUpdate();
                String queryForId = "SELECT id from classsupervisortable ORDER BY id DESC LIMIT 1" ;
                ...
                String queryUpdate = "Update classsupervisortable SET iith_id=?,iith_mailid=? Where id =? ";
