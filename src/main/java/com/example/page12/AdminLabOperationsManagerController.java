...
public class AdminLabOperationsManagerController implements Initializable {
    @FXML
    ...
    void deleteRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0) {
            errorLabel.setVisible(false);
            String query="delete from laboperationsmanagertable where name=? and contactnumber=? " ;
    ...
    void insertRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length()>0&&tfContactNumber.getText().trim().length()>0){
            errorLabel.setVisible(false);
            Connection con = getConnection() ;
            String addQuery = "INSERT INTO laboperationsmanagertable(name,fieldofoperations,appointedby,contactnumber,password) values(?,?,?,?,?)" ;
            PreparedStatement st = con.prepareStatement(addQuery);
            st.setString(1,tfName.getText().trim());
            st.setString(2,"lab");
            st.setString(3,iithId);
            st.setString(4,tfContactNumber.getText().trim());
            st.setString(5,"1234");
            st.executeUpdate();
            String queryForId = "SELECT id from laboperationsmanagertable ORDER BY id DESC LIMIT 1" ;
            ...
            String queryUpdate = "Update laboperationsmanagertable SET iith_id=?,iith_mailid=? Where id =? ";
    ...
    void updateRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0) {
            errorLabel.setVisible(false);
            String query="update laboperationsmanagertable set name=? , contactnumber=? where name=? and contactnumber=?" ;
            Connection con = getConnection();
            PreparedStatement st=con.prepareStatement(query) ;
            st.setString(1,tfName.getText());
            st.setString(2,tfContactNumber.getText());
    ...
    public void filter(ActionEvent event) {
        ...
        if(tfFilterAppointedBy.getText().trim().length()>0 || tfFilterName.getText().trim().length()>0||tfFilterContactNumber.getText().trim().length()>0||tfFilterIithId.getText().trim().length()>0){
            ...
            try{
                ...
                String query = "SELECT * FROM laboperationsmanagertable where iith_id LIKE ? and name LIKE ? and contactnumber LIKE ? and appointedby LIKE ? order by iith_id";
                PreparedStatement st = con.prepareStatement(query);
    ...
    private ObservableList<OperationsManager> getOMList() throws SQLException {
        ObservableList<OperationsManager> operationsManagers = FXCollections.observableArrayList();
        Connection con = getConnection() ;
        String query = "SELECT * FROM laboperationsmanagertable" ;
        try{
