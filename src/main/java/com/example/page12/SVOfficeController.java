...
public class SVOfficeController {
    private Stage stage;
    private Scene scene;
    private Parent root;
    ...
    public void setIithId(String iithId) {
        SVOfficeController.iithId = iithId;
    }
    ...
    public void ObjListView(ActionEvent event) throws IOException {
        root = FXMLLoader.load(requireNonNull(getClass().getResource("SVOfficeObjectList.fxml")));
        stage = (Stage)((Node)event.getSource()).getScene().getWindow();
        SVOfficeObjectListController svOfficeObjectListController= new SVOfficeObjectListController();
        svOfficeObjectListController.setIithId(iithId);
        scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
    }
