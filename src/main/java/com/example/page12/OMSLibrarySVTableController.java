...
public class OMSLibrarySVTableController implements Initializable {
    ...
    void deleteRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0&& tfObjectMaintained.getText().trim().toLowerCase().length()!=0) {
            ...
            String svId = null;
            String query0 = "Select iith_id from librarysupervisortable where name = ? and contactnumber=?and objectmaintained=?";
            ...
            String query="delete from librarysupervisortable where name=? and contactnumber=? and objectmaintained=? " ;
            ...
            String query2= "update libraryitemlist set supervisor='null' where supervisor=? and type=? " ;
    ...
    void updateRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length() !=0 && tfContactNumber.getText().trim().length()!=0 && tfObjectMaintained.getText().trim().toLowerCase().length()!=0 ) {
            ...
            if(tfObjectMaintained.getText().trim().equals(sv.getObjectClassUnderSupervision().trim())){
                errorTypeError.setVisible(false);
                String query="update librarysupervisortable set name=? , contactnumber=? where name=? and contactnumber=? and objectmaintained=?" ;
                Connection con = getConnection();
                PreparedStatement st=con.prepareStatement(query) ;
                st.setString(1,tfName.getText());
                st.setString(2,tfContactNumber.getText());
    ...
    void insertRecord(ActionEvent event) throws SQLException {
        ...
        if(tfName.getText().trim().length()>0&&tfContactNumber.getText().trim().length()>0 && tfObjectMaintained.getText().trim().toLowerCase().length()>0){
            errorInsert.setVisible(false);
            errorLabel.setVisible(false);
            String query = "SELECT * from libraryobjectclasses where objecttype = ? " ;
            ...
            if(resulSet.next()){
                Connection con = getConnection() ;
                String addQuery = "INSERT INTO librarysupervisortable(name,fieldofoperations,objectmaintained,appointedby,contactnumber,password) values(?,?,?,?,?,?)" ;
                PreparedStatement st = con.prepareStatement(addQuery);
                st.setString(1,tfName.getText().trim());
                st.setString(2,"library");
                st.setString(3,tfObjectMaintained.getText().trim().toLowerCase());
                st.setString(4,iithId);
                st.setString(5,tfContactNumber.getText().trim());
                st.setString(6,"1234");
                st.executeUpdate();
                String queryForId = "SELECT id from librarysupervisortable ORDER BY id DESC LIMIT 1" ;
                ...
                String queryUpdate = "Update librarysupervisortable SET iith_id=?,iith_mailid=? Where id =? ";
    ...
    private ObservableList<Supervisor> getSVList() throws SQLException {
        ...
        Connection con = getConnection() ;
        String query = "SELECT * FROM librarysupervisortable" ;
        try{
    ...
    public void filter(ActionEvent event) {
        ...
        if(tfFilterAppointedBy.getText().trim().length()>0 || tfFilterName.getText().trim().length()>0||tfFilterContactNumber.getText().trim().length()>0||tfFilterIithId.getText().trim().length()>0 ||tfFilterObjectMaintained.getText().trim().length()>0){
            ...
            try{
                ...
                Connection con = getConnection() ;
                ObservableList<Supervisor> supervisors = FXCollections.observableArrayList();
                String query = "SELECT * FROM librarysupervisortable where iith_id LIKE ? and name LIKE ? and contactnumber LIKE ? and appointedby LIKE ? and objectmaintained LIKE ? order by iith_id ";
                PreparedStatement st = con.prepareStatement(query);
    ...
    public void OnBack(ActionEvent event) throws IOException{
        ...
        errorFilterLabel.setVisible(false);
        errorInsert.setVisible(false);
        root = FXMLLoader.load(requireNonNull(getClass().getResource("OMSLibraryMode.fxml")));
