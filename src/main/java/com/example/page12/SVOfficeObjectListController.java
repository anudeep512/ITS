...
public class SVOfficeObjectListController implements Initializable {
    //Labels
    @FXML
    ...
    public void setIithId(String iithId) {
        ...
        SVOfficeObjectListController.iithId =iithId ;
    }
    ...
    public void insertRecord(ActionEvent event) throws SQLException, ParseException {
        ...
        if(tfCurrentLocation.getText().trim().length()>0&&tfCompany.getText().trim().length()>0&&tfCompanyId.getText().trim().length()>0&&tfSeller.getText().trim().length()>0&&tfBuyer.getText().trim().length()>0&&tfSupervisor.getText().trim().length()>0&&tfStatus.getText().trim().length()>0&&tfDateOfBuying.getText().trim().length()>0&&tfPrice.getText().trim().length()>0&&tfType.getText().trim().length()>0){
            ...
            String query = "Select * from officesupervisortable where objectmaintained=? and iith_id=? ";
            PreparedStatement st = con.prepareStatement(query);
            st.setString(1,tfType.getText().trim().toLowerCase());
            st.setString(2,tfSupervisor.getText().trim().toLowerCase());
            ResultSet rs = st.executeQuery();
            if(rs.next()){
                String query2 = "Insert into office"+tfType.getText().trim().toLowerCase()+"(type) values('"+tfType.getText().trim().toLowerCase()+"')" ;
                ...
                String queryForId = "SELECT id from office"+tfType.getText().trim().toLowerCase()+" ORDER BY id DESC LIMIT 1" ;
                ResultSet resultSet = st.executeQuery(queryForId) ;
                String iith_id = null ;
                int id = 0;
                while(resultSet.next()){
                    id=resultSet.getInt(1) ;
                    iith_id = tfType.getText().trim().toLowerCase().substring(0,3).toUpperCase() +"OF"+ id;
                }
                System.out.println(id+iith_id);
                String queryAdd = "Insert into officeitemlist(iith_id,type,company,company_id,price,seller,buyer,supervisor,currentlocation,status,dateofbuying,fieldofobject) values(?,?,?,?,?,?,?,?,?,?,?,?) ";
    ...
    private ObservableList<Object> getRecords() throws SQLException {
        ...
        Connection con = getConnection() ;
        String query = "Select * from officeitemlist where supervisor=?";
        PreparedStatement st = con.prepareStatement(query);
    ...
    public void OnBack(ActionEvent event) throws IOException {
        ...
        filterError.setVisible(false);
        objectTypeError.setVisible(false);
        Parent root = FXMLLoader.load(requireNonNull(getClass().getResource("SVOfficeMode.fxml")));
